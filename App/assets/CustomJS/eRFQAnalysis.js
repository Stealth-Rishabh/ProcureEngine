        ...
        success: function (Data, status, jqXHR) {
           ...
           if (_rfqBidType == 'Closed') {
            ...
            if (data[0].vendorNames.length > 0) {
                ...
                strExcel += "<tr><td colspan=6><b>Total</b></td><td colspan=1 style='text-align:center;'><b>" + thousands_separators(totaltargetprice) + "</b></td>";
                ...
                if (ShowPrice == "Y" || totallowestValue == 0) {
                    ...
                    strExcel += "<td>" + thousands_separators((totallowestValue).round(2)) + "</td><td colspan='7'>&nbsp;</td></tr>";
                }
                else {
                                    ...
                                    for (var q = 0; q < data[0].vendorNames.length; q++) {
                                        if (data[0].commercialTerms[s].vendorID == data[0].vendorNames[q].vendorID) {
                                            ...
                                            if (data[0].commercialTerms[s].remarks != '' && data[0].commercialTerms[s].remarks != 'Rejected') {
                                                ...
                                                strExcel += "<td colspan=4>" + encodeURIComponent(data[0].commercialTerms[s].remarks) + "</td>";
...
function fetchrfqcomprativeBoq() {
    ...
    jQuery.ajax({
     ...
     /*       setTimeout(function () {
                ...
                let totalnogst, tquant,nogstprice,thisC;
            for (var v = 0; v < data[0].vendorNames.length; v++){
                    totalnogst=0 ,tquant=0,nogstprice=0
                    $('#tblRFQComprativeBoq>tbody>tr').each(function(i) {
                     debugger
                     thisC = $(this)
                     tquant=parseInt(removeThousandSeperator(thisC.find('.tquant').text()||0))
                     nogstprice=parseInt(removeThousandSeperator(thisC.find(`.VendorPriceNoTax${data[0].vendorNames[v].vendorID}`).text()||0))
                     totalnogst = totalnogst + (nogstprice*tquant)
                     });
                     debugger
                     $(`#totBoxwithgst${data[0].vendorNames[v].vendorID}`).html(thousands_separators(totalnogst))
