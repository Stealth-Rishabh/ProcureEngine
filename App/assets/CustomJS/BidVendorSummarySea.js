        ...
        success: function (data, status, jqXHR) {
            ...
            if (data.length > 0) {
                ...
                if (parseInt(BidTypeID) == 6) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (sname != data[i].shortName) {
                            ...
                            if (PEfaBidForId == 81 || PEfaBidForId == 83) {
                                var str = '<tr id=lowa' + i + ' class=header><td  onclick="fnClickHeader(\'lowa' + i + '\',\'i_expandcollapse' + i + '\',\'forAll\')">' + c + '</td><td id=Sname' + i + '><a href="javascript:void(0);" class="fetchgraphlink" onclick="fetchGraphData(' + data[i].psid + ')"  style="text-decoration:none;">' + data[i].shortName + '</a></td><td class="text-right">' + thousands_separators(data[i].targetPrice) + '</td><td class="text-right">' + thousands_separators(data[i].lastInvoicePrice) + '</td><td class="text-right">' + thousands_separators(data[i].ceilingPrice) + '</td><td class="text-right">' + thousands_separators(data[i].quantity) + '</td><td>' + data[i].uom + '</td><td class="text-right">' + (minimuminc) + '</td>';
                                var strsumm = '<tr id=low' + i + ' class=header><td  onclick="fnClickHeader(\'low' + i + '\',\'i_expandcollapse' + i + '\',\'forAll\')">' + c + '</td><td id=Sname' + i + '><a href="javascript:void(0);" class="fetchgraphlink" onclick="fetchGraphData(' + data[i].psid + ')" style="text-decoration:none;" >' + data[i].shortName + '</a></td><td class="text-right" id=TP1' + i + ' >' + thousands_separators(data[i].targetPrice) + '</td><td class="text-right" id=lastinvoice1' + i + ' >' + thousands_separators(data[i].lastInvoicePrice) + '</td><td class="text-right" id=CP' + i + ' >' + thousands_separators(data[i].ceilingPrice) + '</td><td class="text-right" id=quantity' + i + ' >' + thousands_separators(data[i].quantity) + '</td><td>' + data[i].uom + '</td><td class="text-right" id=Mindec' + i + ' >' + (minimuminc) + '</td>';
                ...
                if (data[0].hideVendor == "Y") {
                    ...
                    $('.fetchgraphlink').removeAttr("onclick");
                }
                else {
...
function fetchparticationQuotes(BidID, VendorID) {
    ...
    if (sessionStorage.getItem("hdnbidtypeid") == 7) {
        url = APIPath + "RemoveParticipatedQuotedPrices/fetchPSQuotedPrices/?BidID=" + BidID + "&VendorID=" + VendorID;
    }
    else if (sessionStorage.getItem("hdnbidtypeid") == 6) {
        url = APIPath + "RemoveParticipatedQuotedPrices/fetchPEFAQuotedPrices/?BidID=" + BidID + "&VendorID=" + VendorID;
    }
    else if (sessionStorage.getItem("hdnbidtypeid") == 9) {
        url = APIPath + "RemoveParticipatedQuotedPrices/fetchFAQuotedPrices/?BidID=" + BidID + "&VendorID=" + VendorID;
    }
    else {
        url = APIPath + "RemoveParticipatedQuotedPrices/fetchCAQuotedPrices/?BidID=" + BidID + "&VendorID=" + VendorID;
    }
    ...
    jQuery.ajax({
        ...
        success: function (data) {
           debugger
            var shortname;
            if (data.length > 0) {
                shortname = (data[0].shortName).replace(/(\r\n|\n|\r)/gm, "");
                removeQuotationPS(shortname, data[0].quotedPrice, data[0].submissionTime, data[0].psid, data[0].psHeaderID);
            }
            jQuery.unblockUI();
        },
...
function deletePSquote() {
    ...
    connection.on("refreshRAQuotes", function (data) {
        ...
        if (JsonMsz[0] == "1" && data[1] == sessionStorage.getItem('UserID')) {
            if ($('#fileToUpload').val() != '') {
                fnUploadFilesonAzure('fileToUpload', AttachementFileName, 'MangeBid/' + BidID);
           ...
           // App.scrollTo(success1, -200);
            ...
            fetchparticationQuotes(BidID, VendorID);
