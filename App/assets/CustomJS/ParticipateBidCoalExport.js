                ...
                if (_isBidStarted == false) {
                    ...
                    if (data[0].bidClosingType == 'A') {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            $('#trquotes' + i).append('<td><button type=button id=AllItembtn' + i + ' class="btn btn-warning clsdisable" onclick=InsUpdQuoteSeaExport(' + i + ')>Submit</button><br/><span id=spanmszA' + i + ' style=color:#a94442>Ok</span></td></tr></tbody>');
...
function InsUpdQuoteSeaExport(index) {
    ...
    //}
        ...
        connection.invoke("RefreshBidParticipationCoal", JSON.stringify(QuoteProduct)).catch(function (err) {
            return console.error(err.toString());
        });
