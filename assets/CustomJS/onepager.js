...
function fillCountryDropDown(dropdownID, countryid) {
  jQuery.ajax({
    ...
    url:
      sessionStorage.getItem("allurlsec") +
      "CustomerRegistration/Country/?CountryID=0",
    cache: false,
    crossDomain: true,
    processData: true,
    dataType: "json",
...
function fillStateDropDown(dropdownID, stateid) {
  ...
  jQuery.ajax({
    ...
    url:
      sessionStorage.getItem("allurlsec") +
      "CustomerRegistration/State/?CountryID=" +
      countryid +
      "&StateID=0",
    cache: false,
    crossDomain: true,
    ...
    success: function (data) {
      var appenddata;
      appenddata +=
        "<option  disabled selected style='display:none;'>State</option>";
...
function fillCityDropDown(dropdownID, cityid) {
  ...
  if (dropdownID == "ddlcompcity") {
    if (jQuery("#ddlcompstate").val() != null) {
      ...
      url =
        sessionStorage.getItem("allurlsec") +
        "CustomerRegistration/City/?StateID= " +
        stateid +
        "&CityID=0";
    }
  }
  jQuery.ajax({
    ...
    success: function (data) {
      var appenddata;
      appenddata +=
        "<option  disabled selected style='display:none;'>City</option>";
...
function handleformsValidation() {
  ...
  $("#signupcomp_frm").validate({
    ...
    errorPlacement: function (error, element) {
      if (element.attr("name") == "signuptnc") {
  ...
  $("#request_demo_frm").validate({
    ...
    errorPlacement: function (error, element) {
      if (element.attr("name") == "requesttnc") {
        // insert checkbox errors after the container
...
function InsRequestDemo() {
  ...
  jQuery.ajax({
    url:
      sessionStorage.getItem("allurlsec") +
      "CustomerRegistration/InsCustomerQueries",
    ...
    success: function (data) {
      ...
      if (data[0].IsSuccess == "1") {
        bootbox.alert(
          "Thanks for showing your interest in our product. One of our team member will contact you within 24 hours for assistance to schedule a demo date and time."
        );
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      if (data[0].IsSuccess == "1") {
        ...
        bootbox.alert(
          "We are glad, you started using our product. Someone from our technical team will call you to assist with implementation of this product and how to use this product."
        );
      } else if (data[0].IsSuccess == "2") {
        $("#sign_up_modal").modal("hide");
        bootbox.alert(
...
function ContactUS() {
  ...
  jQuery.ajax({
    ...
    success: function (data) {
      if (data[0].IsSuccess == "1") {
        bootbox.alert(
          "Message sent successfully. Someone from our team will contact you soon."
        );
      } else if (data[0].Issuccess == "0") {
      }
    },
...
questionaire[3] =
  "Cut down time and effort in negotiation as compare to tradtional process";
questionaire[4] = "Makes your procurement process more transparent";
questionaire[5] = "Easy to use, robust, simple and intuitive";
questionaire[6] =
  "Device agnostic and responsive: Access it from anywhere, at any time and from any device";
...
$("#btn-reasons").click(function (e) {
  ...
  $("#custQuestions").animate(
    ...
    function () {
      $(this)
        .empty()
        .html('<span class="h3">' + questionaire[index + 1] + "</span>")
        .css("opacity", "1");
    }
  );
